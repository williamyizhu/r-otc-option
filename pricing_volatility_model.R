...
library(rgl)
library(RQuantLib)
library(zoo) # used in "historical_volatility_calculation.R" 
library(plyr) # used in "historical_volatility_calculation.R" 
...
#				local variables for "pricing.R"
				...
				if (svalue(aOptionStyle_Geometric)) option_style = c(option_style, "geometric")
				if (svalue(aOptionStyle_Arithmetic)) option_style = c(option_style, "arithmetic")
				
#				group36 items
				contract_year_month = svalue(contract_year_month_obj)				
				historical_volatility_calculation = as.vector(unlist(historical_volatility_obj[1]))
...
#				sync "position_gdf_obj_list" with ".GlobalEnv$position_list", "position_gdf_obj_list" is "Object of class GDf", both "complete.cases()==TRUE" and "complete.cases()==FALSE"
				for (expmth in names(position_gdf_obj_list)) {
					...
					for (pos in 1:dim(position_gdf_obj_list[[expmth]])[1]) {expmth_df = rbind(expmth_df, data.frame(position_gdf_obj_list[[expmth]][pos]))}	
...
#				extract variables and save to local temp variables, which can be access in plots
				for (i in c("f_syn","vol_curr","slope_curr","x1","x2","x0","x3")) eval(parse(text=paste(i,"=vol_theo[[1]]$",i,sep="")))			
...
#				update "glable" object display value
				for (i in c("f_syn","vol_curr","slope_curr","vol_curr_offset","slope_curr_offset")) eval(parse(text=paste("svalue(",i,"_obj)=",i,sep="")))						
...
#----------- plot 1: greek, note: par(mar = c(bottom, left, top, right)), combined_greek_list[[contract_year_month]] may not exist (i.e., ylim=c(0,0)) -----------					
				...
				if (is_position_exist) lines(.GlobalEnv$combined_greek_list[[contract_year_month]][,"strike_price"], .GlobalEnv$combined_greek_list[[contract_year_month]][,paste("position",tolower(svalue(aPositionGreeks)),sep="_")], type="h")
...
#				ATM, reference, synthetic forward price
				...
				legend("bottomright", legend=c("ATM Forward Price","Reference Price","Synthetic Forward Price"), text.col=c("green4","purple","black"), lty=c(2,2,1), col=c("green4","purple","black"), cex=0.75)
...
#				export plot to a file
				...
				dev.print(png, width=1920, height=1080, filename=pfilename)				
				
#------------------------------------- plot Historical Volatility -------------------------------------
#				change graphic device tab
				names(notebook_obj)[2] = "Historical Volatility"
#				switch the plot device for "current" volatility curve
				visible(graphic_historical_volatility) = TRUE				
				plot.new()				
				source(paste(getwd(), "/historical_volatility_calculation.R", sep=""), local=TRUE, echo=FALSE, encoding="GBK")					
...
#----------------------------------------------- main window frame and layout -----------------------------------------------
window = gwindow(paste("Volatility Model ",exchange,".",underlying,sep=""), width=1350, height=800)
#save varialbe list routine function
...
#hot key: Ctrl+s = "\023"
addHandlerKeystroke(window, function(h,...){if(h$key=="\023") window_save_var(h)})

#top level group
mainGroup = ggroup(horizontal=TRUE, container=window)
#"group1" is the paramters area, graphic notebook is in "group2"
group1 = ggroup(horizontal=FALSE, container=mainGroup)
...
group3 = ggroup(horizontal=TRUE, container=group1)
...
group36 = ggroup(horizontal=FALSE, container=group3)
...
#----------------------------------------------- create a menu bar list -----------------------------------------------
#option style "Geometric" and "Arithmetic"
aOptionStyle_Geometric = gcheckbox("Geometric", checked=TRUE, handler=function(h, ...){main_function()})
aOptionStyle_Arithmetic = gcheckbox("Arithmetic", checked=FALSE, handler=function(h, ...){main_function()})
...
#			print(paste("aRevertCurrent = ", svalue(contract_year_month_obj),sep=""))
			contract_year_month = svalue(contract_year_month_obj)
#			svalue(i_obj) <- .GlobalEnv$paramList[,"reference.XXXX"], .GlobalEnv$paramList[,"value.XXXX"] will be updated in main_function()
			for (i in rownames(.GlobalEnv$paramList)) {
				if ((.GlobalEnv$paramList[i,]$type=="gspinbutton") | (.GlobalEnv$paramList[i,]$type=="gslider")) {	
...
#greek risk plot
aPositionGreeks = gradio(c("Vega"), handler=function(h, ...){main_function()})
...
menu_bar_list = list(OTC=list(OptionStyle_Geometric=aOptionStyle_Geometric, OptionStyle_Arithmetic=aOptionStyle_Arithmetic, gsep=gseparator(), GeneratePricing=aGeneratePricing), 
		Volatility=list(SetReference=aSetReference, RevertCurrent=aRevertCurrent, gsep=gseparator(), VolatilitySurface=aVolatilitySurface), 
		Theoretical=list(TheoAutoUpdate=aTheoAutoUpdate, TheoUpdate=aTheoUpdate),
		Position=list(PositionGreeks=aPositionGreeks, gsep=gseparator(), PositionAutoUpdate=aPositionAutoUpdate, PositionUpdate=aPositionUpdate))
...
contract_year_month_obj = gcombobox(contract_year_month_vec, editable=TRUE, handler=function(h,...) {				
#			only update model parameters for "gspinbutton" and "gslider" objects, does not update "glabel" objects			
...
add(gframe('Contract Expiration Month', container=group36), contract_year_month_obj)

#----------------------------------------------- calculation of historical volatility moving average, "gdf" -----------------------------------------------
#eDate="end date", bDays="back calendar days", vPeriod="historical volatility analysis period" 
historical_volatility_obj = gdf(matrix(c("11-30",180,5), nrow=1, byrow=TRUE, dimnames=list(c("V"),c("eDate","bDays","bYears"))))
add(gframe('Historical Volatility Calculation', container=group36), historical_volatility_obj, expand=TRUE)

#----------------------------------------------- number of ITM, OTM strikes and change pct, for both X1 and X2, "gdf" -----------------------------------------------
x1_x2_itm_otm_pct_obj = gdf(matrix(c(0,5,0.01,0,2,0.025), nrow=2, byrow=TRUE, dimnames=list(c("X1","X2"),c("ITM","OTM","Pct"))), container=group36)
#add(gframe('Pricing Strikes X1', container=group36), x1_x2_itm_otm_pct_obj, expand=TRUE)

#----------------------------------------------- number of ITM, OTM strikes and change pct, for both X1 and X2, "gdf" -----------------------------------------------
numerical_siumlation_obj = gdf(matrix(c(100,500,100), nrow=1, byrow=TRUE, dimnames=list(c("V"),c("nSims","minSteps","avgRuns"))))
add(gframe('Numerical Simulation', container=group36), numerical_siumlation_obj, expand=TRUE)
...
graphic_historical_volatility = ggraphics(container=notebook_obj, visible=TRUE)
...
market_maker_group_obj = ggroup(horizontal=TRUE, container=notebook_obj, expand=TRUE)
names(notebook_obj)[3] = "Market Maker"
...
#----------------------------------------------- edit position, "gdf", "gtable" -----------------------------------------------
#create a ggroup obj and include it inside the "notebook_obj"
position_risk_group_obj = ggroup(horizontal=TRUE, container=notebook_obj, expand=TRUE)
...
names(notebook_obj)[4] = "Position & Risk"
...
#1).split "position_data" with factor "expiry_month", "position_data" -> temp_list; 2).sort "x" w.r.t "strike_price", drop the first column "expiry_month", set the rownames to "1:dim(x)[1]"
...
#".GlobalEnv$risk_list" is updated in "position_greeks.R" script
